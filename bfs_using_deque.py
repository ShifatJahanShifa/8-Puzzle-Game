...
class Puzzle:
 ...
 """Check if the current grid is the goal grid."""
    ...
    def solve(self):
        ...
        queue = deque([(self.initial_grid, self.get_blank_pos(self.initial_grid), [])])
        visited = set()
        ...
        while queue:
            current_grid, blank_pos, path = queue.popleft()

            
            print(f"Current Grid after moves: {path}")
            ...
            for direction in directions:
                ...
                if new_grid is not None and tuple(map(tuple, new_grid)) not in visited:
                    ...
                    queue.append((new_grid, self.get_blank_pos(new_grid), path + [direction]))
...
puzzle = Puzzle(initial_grid, goal_grid)
